...
# INPUTS
global is_final, month, year, as_of, bulk_comments
invoice_type = 'rent'
...
def generate_statement(property_data, is_final, month, year, as_of, bulk_comments):
    ...
    for invoice in invoices:
        logging.info(f'invoice[4]: {invoice[4]} / month: {month} / invoice[5]: {invoice[5]} / year: {year} / invoice[8]: {invoice[8]}')
        amount_paid = invoice[8]
        invoice_month = invoice[4]
        invoice_year = invoice[5]
        logging.info(f'Running invoice {invoice[0]}')
        if invoice[8] == 0:
            logging.info('invoice[8] == 0')
            month_number = invoice[4]
            dt = datetime(year=year, month=month_number, day=1)
            # logging.info('date...')
            # logging.info('percentage conversion')
            month_name = dt.strftime("%B")
            payment_date_str = f'UNPAID - {month_name} {year}'
            css_class = 'unpaid' # CSS class for unpaid invoices
        else:
            logging.info('invoice[8] != 0')
            payment_date = invoice[9]
            if invoice_month == month and invoice_year == year:
                logging.info('invoice[4] == month and invoice[5] == year')
                # logging.info('percentage conversion3')
                # logging.info(f'PAYMENT DATE: {payment_date}')
                payment_date_str = payment_date[:10]
            else:
                logging.info('NOT invoice[4] == month and invoice[5] == year')
                month_number = invoice[4]
                dt = datetime(year=year, month=month_number, day=1)
                # logging.info('date2...')
                # logging.info('percentage conversion2')
                month_name = dt.strftime("%B")
                payment_date_str = f'{payment_date} - {month_name} {year}'
                # logging.info(f'payment_date_str {payment_date_str}')
            css_class = '' # No CSS class for paid invoices
        logging.info('Creating unit_ref')
        unit_ref = invoice[3]
        logging.info(f'Unit ref: {unit_ref}')
        logging.info('Creating unit_letter')
        unit_letter = unit_ref[-1].upper()
        logging.info(f'unit_ref: {unit_letter}')
        ...
        if (invoice_month <= month and invoice_year == year) or (invoice_year < year):
            logging.info('(invoice[4] <= month and invoice[5] == year) or (invoice[5] < year)')
            data['table_rows'].append(row)
        elif (((invoice_month > month and invoice_year >= year) or (invoice_month < month and invoice_year > year)) and invoice[8] > 0):
            logging.info('(((invoice[4] > month and invoice[5] >= year) or (invoice[4] < month and invoice[5] > year)) and invoice[8] > 0)')
...
def primary_function(is_final, month, year, as_of, bulk_comments, sql_input):
    blob_urls = {}
    try:
        ...
        for property_data in properties:
            ...
            blob_url = generate_statement(property_data, is_final, month, year, as_of, bulk_comments)
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        logging.info(f'is_final: {is_final} / month: {month} / year: {year} / as_of: {as_of} / bulk_comments: {bulk_comments} / sql_input: {sql_input}')

        statements = primary_function(is_final, month, year, as_of, bulk_comments, sql_input)
